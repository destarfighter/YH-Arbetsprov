...
/// <summary>
/// A Base-Class for a Service, managed with a ServiceLocator to provide global access to all objects in the game.
/// </summary>
class WorldState {
public:
	virtual ~WorldState() { }
...
public:
	...
	virtual bool removeObject(WorldObjectIdentifier id) {
		// Check if key exist, then remove object from all containers
		if (drawableObjects_.find(id) != drawableObjects_.end() ||
			updatableObjects_.find(id) != updatableObjects_.end()) {
	...
	virtual bool findObjectIdByName(std::string name) {
		// Check for object with 'name', return true if found
		for (auto id : worldStateContentIdentities) {
			if (id.name_ == name) {
				return true;
			}
	...
	virtual bool findObjectById(WorldObjectIdentifier id) {
		// Check if object with 'id' exist in either container, return true if found.
		if (drawableObjects_.find(id) == drawableObjects_.end() &&
			updatableObjects_.find(id) == updatableObjects_.end()) {
			return false;
		}
	...
	virtual void addWindow(sf::RenderWindow& applicationWindow) {
		// Add a reference to a RenderWindow
		applicationWindow_ = &applicationWindow;
	}

	virtual sf::RenderWindow& getWindow() {
		// Return the current RenderWindow for the application. used for some input-functions.
