...
void FollowPathState::setAnimationDirection(sf::Vector2i direction) {
	...
	int animIndex = 0;
	if (direction_.x > 0)
		animIndex = ownerAnimator_->getAnimationIndexByName(WALKING_RIGHT_ANIMATION.animName_);
	else if (direction_.x < 0)
		animIndex = ownerAnimator_->getAnimationIndexByName(WALKING_LEFT_ANIMATION.animName_);
	else if (direction_.y > 0)
		animIndex = ownerAnimator_->getAnimationIndexByName(WALKING_DOWN_ANIMATION.animName_);
	else if (direction_.y < 0)
		animIndex = ownerAnimator_->getAnimationIndexByName(WALKING_UP_ANIMATION.animName_);
	ownerAnimator_->changeAnim(animIndex);
}

	void FollowPathState::update(float deltaTime) {
		// move character
		ownerAnimator_->move(direction_.x * movingSpeed_ * deltaTime,
			direction_.y * movingSpeed_ * deltaTime);

		if (getVectorDistance(ownerAnimator_->getPosition(), convertTileToVector(path_[currentTargetIndex_], MAP_WIDTH)) < 0.1f) {

			// snap character to position
			ownerAnimator_->setPosition(static_cast<sf::Vector2f>(convertTileToVector(path_[currentTargetIndex_], MAP_WIDTH)));

			currentTargetIndex_++;
			if (currentTargetIndex_ == path_.size()) {
				// Character has reached goal.
				parent_->setState(new ShowResultState(parent_, ownerAnimator_, true));
				return;
			}

			// get new direction
			auto newDirection = getDirection(static_cast<sf::Vector2u>(ownerAnimator_->getPosition()),
	...
	void FollowPathState::enter() {
		// Set the direction of the character to face the first checkpoint of the path
		direction_ = getDirection(static_cast<sf::Vector2u>(ownerAnimator_->getPosition()),
			convertTileToVector(path_[currentTargetIndex_], MAP_WIDTH));
		setAnimationDirection(direction_);
	}
